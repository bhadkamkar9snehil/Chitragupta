...
class ProfileConfigPatchTests(unittest.TestCase):
    ...
    def test_root_bootstrap_creates_all_plugins_and_is_idempotent(self):
        ...
        self.assertTrue(changed); self.assertEqual(errors, []); self.assertEqual(len(added), 4)
    ...
    def test_root_bootstrap_adds_enabled_under_existing_plugins_block(self):
        ...
        for plugin in ("xstudio-l2-tools", "xstudio-l2-identity", "xstudio-l2-learning", "xstudio-l2-actions"):
            self.assertEqual(text.count(plugin), 1)

    def test_profile_patch_adds_identity_learning_and_actions_without_switching_mem0(self):
        path = self._tmp(
            "memory:\n  provider: mem0\napprovals:\n  deny:\n    - '*sqlcmd*'\nplugins:\n  enabled:\n    - xstudio-l2-tools\n"
        ...
        self.assertEqual(text.count("xstudio-l2-identity"), 1)
    ...
    def test_check_only_reports_change_without_writing(self):
        ...
        for plugin in ("xstudio-l2-identity", "xstudio-l2-learning", "xstudio-l2-actions"):
            self.assertTrue(any(plugin in x for x in added))
