...
def assess_investigation(state: dict[str, Any], *, api_key: str | None = None, sender=None) -> dict[str, Any]:
    solution_candidates = state.get("known_solutions") or []
    solution_criteria = {"NONE": "No known solution is sufficiently supported by current evidence."}
    for i, row in enumerate(solution_candidates[:10]):
        solution_criteria[f"s{i}"] = {
            "title": row.get("title"),
