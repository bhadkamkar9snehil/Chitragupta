...
def approved_case(*, case_id: str, ticket_id: str, run_id: str,
                  response_type: str, action: str) -> str:
    return (
        "---\n"
        f'case_id: "{case_id}"\n'
...
class CapabilityCandidateMiningTests(unittest.TestCase):
    ...
    def _add(self, action: str, ticket: str, index: int):
        (self.vault / f"cases/approved/{index}.md").write_text(
            approved_case(
                case_id=f"c{index}",
                ticket_id=ticket,
                run_id=f"R{index}",
    ...
    def test_two_distinct_human_action_tickets_create_unverified_candidate(self):
        action = (
            "Retry the SAP posting through the supported posting retry path "
            "after confirming the transaction remains pending."
        )
        self._add(action, "T1", 1)
        self._add(action, "T2", 2)
        counts = mod.mine_capability_candidates(self.vault)
        self.assertEqual(counts["created"], 1)
        candidate = json.loads(
            next((self.vault / "actions/candidates").glob("*.json")).read_text(encoding="utf-8")
        )
        ...
        self.assertNotIn("draft_contract", candidate)
        self.assertNotIn("design_requirements", candidate)

    def test_single_ticket_does_not_create_candidate(self):
        self._add(
            "Perform the supported corrective action after validating the live target state.",
            "T1",
            1,
        )
        counts = mod.mine_capability_candidates(self.vault)
    ...
    def test_resolution_cases_are_not_capability_backlog(self):
        ...
        for i, ticket in enumerate(("T1", "T2"), 1):
            (self.vault / f"cases/approved/{i}.md").write_text(
                approved_case(
                    case_id=f"c{i}",
                    ticket_id=ticket,
                    run_id=f"R{i}",
    ...
    def test_new_evidence_preserves_operator_owned_governance(self):
        action = (
            "Retry the supported integration transaction after verifying the failed "
            "live state and duplicate guards."
        )
        self._add(action, "T1", 1)
        self._add(action, "T2", 2)
        ...
        governed["status"] = "shadow_ready"
        governed["draft_contract"] = {
            "id": "xbatch.integration.retry",
            "mode": "shadow",
            "risk": "low",
        }
        governed["governance_history"] = [{
        ...
        self.assertEqual(after["status"], "shadow_ready")
    ...
    def test_corrupt_existing_candidate_fails_closed_without_overwrite(self):
        action = (
            "Reset the supported integration state after verifying the current failed "
            "state and duplicate-prevention condition."
        )
        self._add(action, "T1", 1)
        self._add(action, "T2", 2)
        mod.mine_capability_candidates(self.vault)
    ...
    def test_unchanged_rerun_leaves_candidate_byte_stable(self):
        action = (
            "Perform the supported retry only after verifying the current failure "
            "and duplicate-prevention state."
        )
        self._add(action, "T1", 1)
        self._add(action, "T2", 2)
        mod.mine_capability_candidates(self.vault)
