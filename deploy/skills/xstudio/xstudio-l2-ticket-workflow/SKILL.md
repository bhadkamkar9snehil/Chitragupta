...
metadata:
  hermes:
    tags: [xstudio, helpdesk, l2, investigation, review, learning, actions]
...
The task body identifies the run and ticket. Use those exact identifiers; do not poll or claim another ticket. Expected fields include `run_id`, `ticket_id`, `ticket_no`, `review_cycle`, and `pipeline_stage`.
...
When this run teaches a genuinely reusable lesson, `l2_lesson` may record a concise candidate plus concrete provenance. It creates only `unverified_candidate`; the model cannot promote its own lesson into trusted memory/KB.
...
Use it only after the current root cause/action is known from live evidence. `plan` validates the registered capability's parameter schema and required evidence, records exact run/ticket provenance, hashes the capability contract, and writes an idempotent durable plan under the learning vault. The plan always has `execution_authorized=false`.
...
9. **Record meaningful findings.** Use `save_ledger` for ticket-specific evidence that reviewer/continuation should inspect.
